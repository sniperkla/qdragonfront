...
export async function POST(req) {
  try {
    ...
    if (!accountNumber || !platform || !plan) {
      return new Response(JSON.stringify({ error: 'Account number, platform, and plan are required' }), { status: 400 });
    }

    if (accountNumber.length < 4) {
      return new Response(JSON.stringify({ error: 'Account number must be at least 4 characters' }), { status: 400 });
    }
    ...
    return new Response(JSON.stringify({
      ...
      currency: 'USD',
      status: 'pending_payment',
      message: 'Trading code generated successfully. Please proceed to payment.'
    }), { status: 200 });
