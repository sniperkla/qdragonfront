...
export async function POST() {
  try {
    ...
    const socketEndpoint = process.env.NODE_ENV === 'production'
    ...
    return NextResponse.json({
      ...
      message: nowInitialized ? 'WebSocket server force initialized' : 'Force initialization failed',
      initialized: nowInitialized,
      connectedClients: nowInitialized ? global.__socketIO.engine?.clientsCount || 0 : 0,
      timestamp: new Date().toISOString()
    })
  } catch (error) {
