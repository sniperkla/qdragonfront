...
export async function POST(request) {
  try {
    ...
    console.log('Auth data:', authData ? { id: authData.id, userId: authData.userId } : 'No auth data')
    ...
    if (!codeId || !actualExtendDays || parseInt(actualExtendDays) <= 0) {
      return new Response(JSON.stringify({ error: 'Valid code ID and extend days/plan are required' }), { status: 400 })
    }
    ...
    console.log('User lookup:', user ? { id: user._id, username: user.username } : 'User not found')
    ...
    console.log('Code request found:', codeRequest ? { id: codeRequest._id, userId: codeRequest.userId, code: codeRequest.code } : 'Not found')
    ...
    console.log('Customer account lookup:', {
      ...
      found: customerAccount ? { id: customerAccount._id, license: customerAccount.license, status: customerAccount.status } : 'Not found'
    })
    ...
    if (!workingCustomerAccount) {
      ...
      if (totalAccounts > 0) {
        const sampleAccounts = await CustomerAccount.find({}).limit(3).select('license user status')
      ...
      try {
        ...
        console.log('Created missing customer account:', workingCustomerAccount._id)
      } catch (createError) {
    ...
    if (workingCustomerAccount.status !== 'valid') {
      return new Response(JSON.stringify({ error: 'Cannot extend expired or suspended accounts' }), { status: 400 })
    }
    ...
    if (existingRequest) {
      return new Response(JSON.stringify({
        error: 'You already have a pending extension request for this code. Please wait for admin approval.'
      }), { status: 400 })
    }
    ...
    return new Response(JSON.stringify({
      success: true,
      message: 'Extension request submitted successfully. Please wait for admin approval.',
      requestId: extensionRequest._id,
      licenseCode: codeRequest.code,
      currentExpiry: workingCustomerAccount.expireDate,
      requestedDays: parseInt(actualExtendDays),
