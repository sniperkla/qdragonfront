...
export async function POST(req) {
  try {
    ...
    if (!codeId) {
      return new Response(JSON.stringify({ error: 'codeId required' }), { status: 400 })
    }
    ...
    if (codeReq.activationEmailResentCount >= 3) {
      return new Response(JSON.stringify({ error: 'Resend limit reached (3)' }), { status: 429 })
    }

    const userDoc = await User.findById(codeReq.userId).select('email username preferredLanguage')
    if (!userDoc?.email) {
      return new Response(JSON.stringify({ error: 'User email unavailable' }), { status: 400 })
    }

    const result = await sendLicenseActivatedEmail(userDoc.email, userDoc.username, {
