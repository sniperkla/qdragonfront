...
export default function AdminPage() {
  ...
  const [newItemNotification, setNewItemNotification] = useState({ show: false, message: '' })
      ...
      interval = setInterval(async () => {
        try {
          ...
          if (codesResponse.ok && customersResponse.ok) {
            ...
            // Check for new codes
            if (codesData.codes && codesData.codes.length > lastCodeCount && lastCodeCount > 0) {
              const newCodesCount = codesData.codes.length - lastCodeCount
              showNotification(`🆕 ${newCodesCount} new trading code${newCodesCount > 1 ? 's' : ''} generated!`)
              // Play notification sound
              playNotificationSound()
            }
            
            // Check for new customers
            if (customersData.accounts && customersData.accounts.length > lastCustomerCount && lastCustomerCount > 0) {
              const newCustomersCount = customersData.accounts.length - lastCustomerCount
              showNotification(`🎉 ${newCustomersCount} new customer account${newCustomersCount > 1 ? 's' : ''} activated!`)
              // Play notification sound
              playNotificationSound()
            }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCodes(prev => prev.map(code => 
          code._id === codeId 
            ? { ...code, status: newStatus, ...(newStatus === 'paid' && { paidAt: new Date() }) }
            : code
        ))
        
        // If customer account was created, add it directly to the state
        if (data.customerAccountCreated && data.customerAccount) {
          console.log('Adding new customer account to state:', data.customerAccount)
        ...
        showToast(`Code status updated to ${newStatus.replace('_', ' ').toUpperCase()}`, 'success')
      } else {
  ...
  const updateCustomerStatus = async (accountId, newStatus) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast(`Customer account ${newStatus === 'suspended' ? 'suspended' : newStatus === 'valid' ? 'reactivated' : newStatus}`, 'success')
      } else {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const endpoint = type === 'code' ? '/api/admin/codes' : '/api/admin/customers'
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Q-DRAGON Admin</h1>
            <p className="text-gray-600">Enter admin key to access payment management</p>
          </div>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Q-DRAGON Admin Panel</h1>
              <p className="text-gray-600">Manage trading code payments and activations</p>
              {autoRefresh && (
            ...
            <div className="flex items-center gap-4">
              <label className="flex items-center cursor-pointer">
                <input
                  ...
                  className="sr-only"
                />
                <div className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${autoRefresh ? 'bg-green-600' : 'bg-gray-300'}`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
          {['all', 'pending_payment', 'paid', 'activated', 'expired'].map(status => {
            const count = status === 'all' ? codes.length : codes.filter(c => c.status === status).length
            return (
              <div key={status} className="bg-white rounded-xl shadow-lg p-6">
                <div className="text-2xl font-bold text-gray-900">{count}</div>
                <div className="text-sm text-gray-600 capitalize">
                  {status === 'all' ? 'Total Codes' : status.replace('_', ' ')}
                </div>
              </div>
            )
          })}
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-sm font-medium text-gray-900">Plan</th>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex gap-2">
                        {code.status === 'pending_payment' && (
                          <button
                            onClick={() => updateCodeStatus(code._id, 'paid')}
                        ...
                        {code.status === 'paid' && (
                          <button
                            onClick={() => updateCodeStatus(code._id, 'activated')}
                            disabled={updating[code._id]}
                        ...
                        {code.status !== 'cancelled' && code.status !== 'expired' && (
                          <button
                            onClick={() => updateCodeStatus(code._id, 'cancelled')}
                            disabled={updating[code._id]}
                        ...
                        {code.status === 'cancelled' && (
                          <button
                            onClick={() => handleDeleteClick('code', code._id, code.code)}
                            disabled={updating[code._id]}
          ...
          {filteredCodes.length === 0 && (
            <div className="text-center py-12 text-gray-500">
              ...
              <p className="text-sm">No trading codes match your current filter</p>
            </div>
          )}
        </div>
          </>
                ...
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-left text-sm font-medium text-gray-900">Status</th>
                      <th className="px-6 py-4 text-left text-sm font-medium text-gray-900">Activated</th>
                      <th className="px-6 py-4 text-left text-sm font-medium text-gray-900">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200">
                    {customers
                      ...
                      .map((customer) => (
                        <tr key={customer._id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 text-sm text-gray-500">
                            {new Date(customer.activatedAt).toLocaleDateString()}
                          </td>
                          <td className="px-6 py-4">
                            <div className="flex gap-2">
                              {customer.status === 'valid' && (
                                <button
                                  onClick={() => updateCustomerStatus(customer._id, 'suspended')}
                              ...
                              {customer.status === 'suspended' && (
                                <button
                                  onClick={() => updateCustomerStatus(customer._id, 'valid')}
                                  disabled={updating[customer._id]}
                                  className="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded text-xs disabled:opacity-50"
                                >
                                  {updating[customer._id] ? '...' : 'Reactivate'}
                                </button>
                              )}
                              {customer.status === 'expired' && (
                                <button
                              ...
                              {customer.status === 'suspended' && (
                                <button
                                  onClick={() => handleDeleteClick('customer', customer._id, customer.license)}
                                  disabled={updating[customer._id]}
                                  className="bg-gray-800 hover:bg-gray-900 text-white px-3 py-1 rounded text-xs disabled:opacity-50"
                                >
          ...
          <div className="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full mx-4">
            <div className="text-center mb-6">
              ...
              <h3 className="text-lg font-bold text-gray-900 mb-2">
                Delete {deleteConfirmation.type === 'code' ? 'Trading Code' : 'Customer Account'}
              </h3>
            ...
            <div className="flex gap-4">
              ...
              <button
                ...
                disabled={confirmText !== 'DELETE' || updating[deleteConfirmation.id]}
              ...
              >
                {updating[deleteConfirmation.id] ? 'Deleting...' : 'Delete Forever'}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-green-500 text-white px-6 py-4 rounded-lg shadow-lg border-l-4 border-green-600 animate-bounce">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm opacity-90">Check the tables below for details</p>
              </div>
              <button
                onClick={() => setNewItemNotification({ show: false, message: '' })}
